...
void mqtt_comm_publish(const char *topic, const uint8_t *data, size_t len) {
    // criptografa a mensagem mqtt
    uint8_t cryptedBuffer[128];
    xor_encrypt((uint8_t *)data, cryptedBuffer, strlen(data), 42);
    // Envia a mensagem MQTT
    err_t status = mqtt_publish(
    client, // Instância do cliente
    topic, // Tópico de publicação
    cryptedBuffer, // Dados a serem enviados
...
// Callback chamado para receber o payload da mensagem
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    printf("MQTT payload recebido: ");
    xor_encrypt((uint8_t *)data, message.message, strlen(data), 42);
    message.message[strlen(message.message)]='\0';
    printf("%s\n",message.message);

    on_message(message.topic,message.message);
...
void mqtt_comm_subscribe(const char *topic) {
    ...
    // Registra os callbacks de mensagem recebida
    mqtt_set_inpub_callback(client, mqtt_incoming_publish_cb, mqtt_incoming_data_cb, NULL);

    // Faz a inscrição no tópico com QoS 0
    status = mqtt_subscribe(client, topic, 0, NULL, NULL);

    if (status == ERR_OK) {
        printf("Inscrito no tópico MQTT '%s'\n", topic);
    } else {
